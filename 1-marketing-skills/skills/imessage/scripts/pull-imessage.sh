#!/usr/bin/env bash
# Brand Voice — pull iMessage history into per-person daily .md files.
# Usage: pull-imessage.sh [--days N] [--rebuild]
#   --days N    how many days back to pull (default 90)
#   --rebuild   overwrite existing day files (default skips dates already written)
#
# Reads ~/Library/Messages/chat.db read-only. Mac will prompt for Full Disk
# Access the first time — grant it via System Settings → Privacy & Security.
# Only outbound messages (is_from_me=1) are written, since we're capturing
# the user's voice, not their friends'.
...
# iMessage stores dates as nanoseconds since 2001-01-01 UTC. The Unix epoch
# offset is 978307200. Cutoff = (now - DAYS) in iMessage native units.
...
while IFS=$'\t' read -r handle person; do
  [ -z "$handle" ] && continue
  PERSON_DIR="$ROOT/comms/$person"
  mkdir -p "$PERSON_DIR"

  # Pull all outbound messages to this handle in the window, ordered by date.
  # date column is iMessage-native; convert to ISO date for grouping.
  ROWS=$(sqlite3 -separator $'\x1f' "file:${CHAT_DB}?mode=ro" "
    ...
    ORDER BY m.date ASC;
  " 2>&1) || {
    echo "Query failed for $handle ($person): $ROWS" >&2
    continue
  }

  [ -z "$ROWS" ] && { echo "  $person ($handle): no messages in last $DAYS days"; continue; }
  ...
  while IFS=$'\x1f' read -r d t txt; do
    ...
    if [ "$d" != "$CURRENT_DATE" ]; then
      ...
      if [ -f "$CURRENT_FILE" ] && [ "$REBUILD" -eq 0 ]; then
        CURRENT_FILE=""
        continue
      fi
      printf "# Comms with %s — %s\n\n" "$person" "$d" > "$CURRENT_FILE"
    fi
  ...
  echo "  $person ($handle): wrote $COUNT messages"
  TOTAL=$((TOTAL+COUNT))
done < "$CONTACT_LIST"

echo
echo "Done. $TOTAL outbound messages digested → $ROOT/comms/"
